...
class Session_formation
{
    ...
    * @return string
    */
    public function dateModifiee($date)
    {
        $dateString = $date->format('d-m-Y');
        $tab = explode("-",$dateString);
        $jour=$tab[0];
        $mois=$tab[1];
        $annee=$tab[2];
        $dateModifiee = $jour . "/".$mois."/".$annee;
        return $dateModifiee;
    }    
    ...
    */
    ...
    {
        return ($this->formation->getLibelle() . " - " . $this->formation->getNiveau() . " - " . $this->dateModifiee($this->dateDebut));
    }
